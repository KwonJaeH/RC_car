...
void obstacle(long distance, long L_dist,long R_dist){
   ...
   else if( distance < 60 && speed  <= 200){ // 앞에 물체가 있다면 속도를 서서히 줄이기
        Pause();
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(ENCODER),Isr,RISING);
}
