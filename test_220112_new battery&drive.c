...
void Pause(){
    ...
    while(1){
          if(speed >= 30){speed -= 30;}else if(speed == 0){break;}else{ speed = 0;}
...
void turn_left(int val,int speed){
    // 좌회전
    digitalWrite(Dir1Pin_R, LOW);         // 오른쪽 바퀴들 앞으로
    digitalWrite(Dir2Pin_R, HIGH);
    analogWrite(SpeedPin_R, speed);
...
void turn_right(int val,int speed){
  //우회전
...
void back_straight(){
  // 후진
    digitalWrite(Dir1Pin_R, HIGH);         // 오른쪽 바퀴들 뒤로
    digitalWrite(Dir2Pin_R, LOW);
    analogWrite(SpeedPin_R, DEFAULT_SPEED);
     
    digitalWrite(Dir6Pin_L, LOW);         // 왼쪽 바퀴들 뒤로
    digitalWrite(Dir7Pin_L, HIGH);
    analogWrite(SpeedPin_L, DEFAULT_SPEED);
...
void obstacle(long distance, long L_dist,long R_dist){
   ...
   Serial.println(" Cm");
  /*
   if(distance < 20 && speed >= 200){  // 속도가 빠를 때 갑자기 나왔을 때  *가정 : 앞에 장애물의 폭은 작다 ( 현재 ex, 손)
      if(L_dist > R_dist && L_dist > 20){ 
          little_left(500);
          go_straight(200);
          delay(200);
          little_right(500);
          Serial.print("좌 - 우");
        }else if(L_dist<R_dist && R_dist > 20){
          little_right(500);
          go_straight(250);
          delay(200);
          little_left(500);
   ...
   else */if( distance < 50 && speed  <= 200){ // 앞에 물체가 있다면 속도를 서서히 줄이기
        Pause();
        ...
        if(L_dist > R_dist && L_dist > 30){ 
          turn_left(LEFT,DEFAULT_SPEED);
        }else if(L_dist<R_dist && R_dist > 30){
          turn_right(RIGHT,DEFAULT_SPEED);
        }else if(L_dist > 30){
          turn_left(LEFT,DEFAULT_SPEED);
        }else{
          back_straight();
...
void right_rotation(double car_angle){
     
     double val = car_angle*(RIGHT_ROT/365); 
...
void left_rotation(double car_angle){
   
     double val = car_angle*(LEFT_ROT/365);
...
void rotate_to_return(double x, double y){
  ...
  // val = delay 할 시간
    ...
    if( where == 1){           // 1사분면에서 각 x, y 가 증가( 출발점에서 바깥으로)할 때, 감소(출발점으로 안으로)할 때 돌릴 방향 결정
      if(x_state == 1){
          car_angle = 180 - angle;
          right_rotation(car_angle);
      }else if(y_state == 1){
          car_angle = 90 + angle;
          left_rotation(car_angle);
      }else if(x_state == -1){
          car_angle = angle;
          left_rotation(car_angle);
      }else if(y_state == -1){
...
void loop(){
  ...
  */
  ...
  }else if(millis() > 5000){  // 그냥 배터리 부착 시간 + 해서 5초 뒤 출발 가정
